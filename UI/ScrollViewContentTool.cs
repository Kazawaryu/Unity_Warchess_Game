...
public class ScrollViewContentTool : MonoBehaviour
{
    ...
    bool _flash = false;
    GameObject _textLine;
    GameObject _rollBar;
    public void addTextLine(string innerString)
    {
        GameObject _nextLine = Instantiate(_textLine);
        _nextLine.transform.SetParent(this.transform);
        Text m_text = _nextLine.GetComponent<Text>();
        m_text.text = innerString;
        _nextLine.GetComponent<RectTransform>().anchoredPosition = Vector3.zero + Vector3.down * this.transform.childCount * (m_cellSize.y + m_Spacing.y);
    }
    ...
    private void Start()
    {
        _textLine = Resources.Load<GameObject>("TextLine");
        _rollBar = GameObject.Find("DetailInformation/ScrollbarVertical");
    }
    ...
    private void Update()
    {
        if (_flash)
        {
            _rollBar.GetComponent<Scrollbar>().value = 0;
            _flash = false;
        }
