...
public class SkillBoard : MonoBehaviour
{
    ...
    public int m_tempChildCount = 0;

    GameObject _SkillButton;


    public void addSkillButton(ISkill skill, Character character, MyTurn turn)
    {
        GameObject _nextButton = Instantiate(_SkillButton);
        _nextButton.transform.SetParent(this.transform);
        SkillButton _nextComp = _nextButton.GetComponent<SkillButton>();
        _nextComp.Constructors(skill, m_tempChildCount++, character, turn);
        _nextButton.GetComponent<RectTransform>().localPosition = new Vector3(0, -this.transform.childCount * (m_cellSize.y + m_Spacing.y), 0);
        Text _skillName = _nextButton.transform.Find("SkillName").GetComponent<Text>();
    ...
    private void Start()
    {
        _SkillButton = Resources.Load<GameObject>("SkillButton");
    }
    ...
    private void UpdateContentSize(int _count)
    {
        ...
        else if (m_barType == ScrollBarType.Vertical)
        {
            ...
            {
                m_rectTransform.sizeDelta = new Vector2(0, _count * (m_cellSize.y + m_Spacing.y) + 15);
